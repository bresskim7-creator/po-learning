...
function validateGrowthValues(date, h, w) {
  ...
  // [P3] 미래 날짜 차단 (오늘은 허용)
  var today = todayIsoLocalGrowth();
  if (date > today) return '오늘 이후 날짜는 입력할 수 없어요. 측정한 날짜를 확인해주세요.';
...
// [P3] 입력일 기준 이전·이후 인접 기록 모두 비교 + 키 감소는 기간 무관 경고
function checkGrowthChangeWarning(date, h, w, excludeId) {
  ...
  for (var i = 0; i < asc.length; i++) {
    if (asc[i].date < date) {
      prev = asc[i];
    } else if (asc[i].date > date) {
      next = asc[i];
      break;
    }
  }

  function evalNeighbor(neighbor, direction) {
    if (!neighbor) return null;
    var dayDiff = Math.round(Math.abs(Date.parse(date) - Date.parse(neighbor.date)) / msDay);
    var dh = h - neighbor.heightCm;
    var dw = w - neighbor.weightKg;
    var absDh = Math.abs(dh);
    var absDw = Math.abs(dw);

    var heightDecreaseAbs = 0;
    if (direction === 'prev' && dh < 0) heightDecreaseAbs = -dh;
    if (direction === 'next' && dh > 0) heightDecreaseAbs = dh;

    var triggered = false;
    if (dayDiff <= GROWTH_LIMITS.warnWindowDays) {
      if (absDh >= GROWTH_LIMITS.warnDeltaHeightCm) triggered = true;
      if (absDw >= GROWTH_LIMITS.warnDeltaWeightKg) triggered = true;
    }
    if (heightDecreaseAbs >= 1) triggered = true;
    ...
    return {
      ...
      dh: dh,
      dw: dw,
      heightDecreaseAbs: heightDecreaseAbs
    };
  }

  var prevWarn = evalNeighbor(prev, 'prev');
  var nextWarn = evalNeighbor(next, 'next');
  if (!prevWarn && !nextWarn) return null;
  if (prevWarn && nextWarn) {
    var prevScore = Math.max(Math.abs(prevWarn.dh), Math.abs(prevWarn.dw), prevWarn.heightDecreaseAbs * 2);
    var nextScore = Math.max(Math.abs(nextWarn.dh), Math.abs(nextWarn.dw), nextWarn.heightDecreaseAbs * 2);
    return prevScore >= nextScore ? prevWarn : nextWarn;
  }
  return prevWarn || nextWarn;
}
...
function submitGrowthForm() {
  ...
  if (warn) {
    ...
    var directionTxt = warn.direction === 'prev' ? '전' : '후';
    var prefix = '';
    if (warn.heightDecreaseAbs >= 3) {
      prefix = '⚠ 키가 ' + warn.heightDecreaseAbs.toFixed(1) + 'cm 줄어든 것 같아요.\n키는 보통 줄지 않으니 측정값을 다시 확인해주세요.\n\n';
    } else if (warn.heightDecreaseAbs >= 1) {
      prefix = '키가 조금 줄어든 것 같아요. 측정값을 한 번 더 확인해주세요.\n\n';
    }
    var ok = confirm(
      prefix +
      warn.dayDiff + '일 ' + directionTxt + '(' + formatGrowthDate(warn.neighbor.date) + ') 대비\n' +
      '키 ' + dhTxt + ' / 몸무게 ' + dwTxt + '\n' +
...
async function retryPendingGrowth() {
  ...
  for (var i = 0; i < pending.length; i++) {
    var item = pending[i];
    // [P1] HTTP 200 + JSON {status:'ok', type:'growth'} 일 때만 성공 처리
    var ok = false;
    try {
      ...
      if (resp.ok) {
        try {
          ...
          if (result && result.status === 'ok' && result.type === 'growth') {
            ok = true;
          }
        } catch(jsonErr) {
          ok = false;
  ...
  // [P2] 펜딩에 남은 record id의 syncedAt은 null로 유지 (이전 성공으로 채워진 syncedAt이 최신 실패를 가리는 문제 방지)
  if (stillPending.length > 0) {
    var pendingIds = {};
    for (var k = 0; k < stillPending.length; k++) {
      var pid = stillPending[k].record && stillPending[k].record.id;
      if (pid) pendingIds[pid] = true;
    }
    var all2 = loadGrowthRecords();
    var changed = false;
    for (var m = 0; m < all2.length; m++) {
      if (pendingIds[all2[m].id] && all2[m].syncedAt) {
        all2[m].syncedAt = null;
        changed = true;
      }
    }
    if (changed) saveGrowthRecords(all2);
  }
